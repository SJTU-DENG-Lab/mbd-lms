...
class DataCollatorWithPadding(DataCollator):
    ...
    def __call__(self, features: Sequence[Dict[str, "torch.Tensor"]]) -> Dict[str, "torch.Tensor"]:
        ...
        for key in batch.keys():
            ...
            if key in ["input_ids", "attention_mask", "position_ids", "images_seq_mask", "buffer_sizes"]:
...
class MakeMicroBatchCollator(DataCollator):
    ...
    def __call__(self, features: Sequence[Tuple[Dict[str, "torch.Tensor"]]]) -> List[Dict[str, "torch.Tensor"]]:
        expanded_features: List[Dict[str, "torch.Tensor"]] = []
        for feature in features:
            # Support transform outputs that expand one sample to multiple variants.
            if isinstance(feature, dict):
                expanded_features.append(feature)
            elif isinstance(feature, (list, tuple)):
                expanded_features.extend(feature)
            else:
                raise TypeError(f"Unsupported feature type in MakeMicroBatchCollator: {type(feature)}")

        if self.num_micro_batch <= 0:
            raise ValueError(f"num_micro_batch must be positive, got {self.num_micro_batch}")
        if len(expanded_features) < self.num_micro_batch:
            raise ValueError(
                f"Expanded feature count {len(expanded_features)} is smaller than num_micro_batch {self.num_micro_batch}."
            )

        # Split expanded features into exactly num_micro_batch chunks.
        base = len(expanded_features) // self.num_micro_batch
        remainder = len(expanded_features) % self.num_micro_batch
        micro_batches = []
        start = 0
        for i in range(self.num_micro_batch):
            chunk_size = base + (1 if i < remainder else 0)
            end = start + chunk_size
            micro_batches.append(self.internal_data_collator(expanded_features[start:end]))
            start = end
