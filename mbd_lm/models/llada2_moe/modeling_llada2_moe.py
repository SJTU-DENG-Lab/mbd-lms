...
class LLaDA2MoeModel(LLaDA2MoePreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        elif self._use_flex_attention:
            if not isinstance(attention_mask, BlockMask):
                raise TypeError("attention_mask should be `torch.nn.attention.flex_attention.BlockMask` when attention implementation is set to `flex_attention`")
            attention_mask = attention_mask
        else:
