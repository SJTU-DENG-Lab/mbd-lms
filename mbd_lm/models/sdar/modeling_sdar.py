...
class SDARAttention(nn.Module):
    ...
    def forward(
        ...
        kvcache_update_len: Optional[int] = None,
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if kvcache_update_len is not None and kvcache_update_len > 0:
            cache_kwargs = {"sin": sin[:,:kvcache_update_len,:], "cos": cos[:,:kvcache_update_len,:], "cache_position": cache_position[:kvcache_update_len]}
            new_key_states, new_value_states = past_key_value.update(key_states[:,:,:kvcache_update_len, :], value_states[:,:,:kvcache_update_len, :], self.layer_idx, cache_kwargs)
            key_states = torch.cat([new_key_states, key_states[:,:,kvcache_update_len:,:]], dim=2)
            value_states = torch.cat([new_value_states, value_states[:,:,kvcache_update_len:,:]], dim=2)
        elif past_key_value is not None and kwargs.get("store_kv", False):
...
class SDARFlexAttention(SDARAttention):
    """SDAR attention with flex_attention backend for BlockMask (e.g. block diffusion)."""
...
ATTENTION_CLASSES = {
    "flash_attention_2": SDARAttention,
    "flex_attention": SDARFlexAttention,
}
...
class SDARDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: SDARConfig, layer_idx: int):
        ...
        attn_cls = ATTENTION_CLASSES.get(attn_impl, SDARAttention)
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        hidden_states, self_attn_weights = self.self_attn(
            ...
            kvcache_update_len=kvcache_update_len,
        ...
        if self.config._attn_implementation == "flex_attention":
            if isinstance(attention_mask, torch.Tensor):
                if attention_mask.dim() == 2:
                    ...
                    mask_2d = attention_mask.to(dtype=torch.bool)
                    attention_mask = create_block_mask(
                        lambda b, h, q_idx, kv_idx: mask_2d[q_idx, kv_idx],
                        B=None,
                        H=None,
                        Q_LEN=seq_len_q,
                        KV_LEN=seq_len_kv,
                    ...
                    batch_size, seq_len_q, seq_len_kv = attention_mask.shape
                    assert seq_len_q == seq_len_kv, f"got {attention_mask.shape=}"
                    mask_3d = attention_mask.to(dtype=torch.bool)
                    attention_mask = create_block_mask(
                        lambda b, h, q_idx, kv_idx: mask_3d[b, q_idx, kv_idx],
                        B=batch_size,
                        H=None,
                        Q_LEN=seq_len_q,
                        KV_LEN=seq_len_kv,
                    ...
                    batch_size, num_heads, seq_len_q, seq_len_kv = attention_mask.shape
                    assert seq_len_q == seq_len_kv, f"got {attention_mask.shape=}"
                    mask_4d = attention_mask.to(dtype=torch.bool)
                    if num_heads == 1:
                        mask_3d = mask_4d[:, 0]
                        attention_mask = create_block_mask(
                            lambda b, h, q_idx, kv_idx: mask_3d[b, q_idx, kv_idx],
                        ...
                        attention_mask = create_block_mask(
                            lambda b, h, q_idx, kv_idx: mask_4d[b, h, q_idx, kv_idx],
                            B=batch_size,
                            H=num_heads,
                            Q_LEN=seq_len_q,
                            KV_LEN=seq_len_kv,
                    ...
                    raise ValueError(
                        f"Unsupported attention_mask dim for flex_attention conversion: {attention_mask.dim()}"
                    )
            else:
