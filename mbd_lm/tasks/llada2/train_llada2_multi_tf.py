...
from veomni.utils.arguments import parse_args, save_args
...
ModelRegistry.register_modeling_path("mbd_lm.models.llada2_moe")
...
def main():
    ...
    if mask_token not in tokenizer.get_vocab():
        ...
        logger.info_rank0(f"Added mask token {mask_token} to tokenizer, token_id={tokenizer.convert_tokens_to_ids(mask_token)}")
    ...
    if args.data.data_type == "conversation":
        ...
        # TODO: implement tokenid data type
    ...
    if len(tokenizer) > student_model.get_input_embeddings().weight.shape[0]:
        logger.info_rank0(f"Resizing model embeddings from {student_model.get_input_embeddings().weight.shape[0]} to {len(tokenizer)}")
    ...
    if args.train.use_distill:
        ...
        if len(tokenizer) > teacher_model.get_input_embeddings().weight.shape[0]:
            logger.info_rank0(f"Resizing teacher model embeddings from {teacher_model.get_input_embeddings().weight.shape[0]} to {len(tokenizer)}")
    ...
    # =======================================================
    # Prepare optimizer and lr scheduler for **student model** (only for student model, not for teacher model)
    # =======================================================
    ...
    if args.train.load_checkpoint_path:
        student_state = {"model": student_model, "optimizer": optimizer, "extra_state": {}}  # cannot be None
    ...
    # =======================================================
    # Build Multi-BD attention mask (full seq = noisy + clean = 2 * max_seq_len)
    # =======================================================
    if args.train.dual_bd_mode:
        block_size = args.train.block_size
        multi_bd_full_len = 2 * args.data.max_seq_len  # x_t len + x_0 len

        def mask_flag_fn_generator(mask_flag_fn: Callable, buffer_ids: torch.Tensor = None) -> Callable:
            if buffer_ids is None:
                ...
                # buffer_ids from dataset: [seq_len] or [B, seq_len]; mask expects [2*seq_len]
            ...
            return fn
        student_multi_bd_attn_mask_prototype = None # sample-specific, generated in each step
        if args.train.use_distill:
            teacher_multi_bd_attn_mask_prototype = multi_bd_attn_mask_generator(
                mask_flag_fn_generator(multi_bd_attn_mask_teacher),
    ...
    for epoch in range(start_epoch, args.train.num_train_epochs):
        ...
        for _ in range(start_step, args.train.train_steps):
            ...
            # Effective loss weights: step schedule and/or loss-aware balance, then optional normalize to sum 1
            ...
            for micro_batch in micro_batches:
                ...
                if args.train.dual_bd_mode:
                    ...
                    def micro_batch_generator(attn_mask):
                        use_flex = args.model.attn_implementation == "flex_attention"
                        if use_flex:
                            device = get_device_type()
                            # attn_mask may be [1,1,Q,KV] or [B,1,Q,KV]; support per-sample masks
                            if attn_mask.dim() == 4 and attn_mask.shape[0] > 1:
                                boolean_3d = rearrange(attn_mask == 0.0, "b 1 q k -> b q k").to(device)
                                def block_attn_mask_mod(batch_idx, head_idx, q_idx, kv_idx):
                                    return boolean_3d[batch_idx, q_idx, kv_idx]
                            else:
                                boolean_2d = rearrange(attn_mask == 0.0, "1 1 q k -> q k").to(device)
                                def block_attn_mask_mod(batch_idx, head_idx, q_idx, kv_idx):
                            ...
                            block_mask = create_block_mask(
                                block_attn_mask_mod,
                                B=batch_size,
                                H=student_model_config.num_attention_heads,
                    ...
                    # Student mask is sample-specific: build per sample when batch_size > 1
                    ...
                    if buf_ids.dim() == 1 or buf_ids.shape[0] == 1:
                        student_multi_bd_attn_mask_prototype = multi_bd_attn_mask_generator(
                            mask_flag_fn_generator(multi_bd_attn_mask_student, buffer_ids=buf_ids),
                            args.train.enable_mixed_precision,
                        )
                    else:
                        student_masks = [
                            multi_bd_attn_mask_generator(
                                mask_flag_fn_generator(multi_bd_attn_mask_student, buffer_ids=buf_ids[b]),
                                args.train.enable_mixed_precision,
                            )
                    ...
                    if args.train.use_distill:
                        student_micro_batch = micro_batch_generator(student_multi_bd_attn_mask_prototype)
                        teacher_micro_batch = micro_batch_generator(teacher_multi_bd_attn_mask_prototype)
                    else:
                        student_micro_batch = micro_batch_generator(student_multi_bd_attn_mask_prototype)
                        teacher_micro_batch = None
                else:
                    # Fallback: non dual-bd mode (keep interface consistent)
                    micro_batch["attention_mask"] = None
                    student_micro_batch = {
                        k: micro_batch[k]
                        for k in ("input_ids", "position_ids", "attention_mask")
                        if k in micro_batch
                    }
                    if args.train.use_distill:
                        teacher_micro_batch = dict(student_micro_batch)
                    else:
                        teacher_micro_batch = None
                ...
                with model_fwd_context:
                    ...
                    if args.train.use_distill:
                        # 1) teacher forward (no grad), serially executed
                        with torch.no_grad():
                    ...
                    # 3) CE loss (same slicing / shifting behavior as before)
                    if args.train.same_token_labels:
                        unscaled_ce = F.cross_entropy(
                            student_noisy_logits.view(-1, student_noisy_logits.shape[-1]),
                            gt_labels.view(-1),
                            reduction="none",
                        )
                        ce_loss = unscaled_ce.sum() / (gt_labels != -100).sum() / len(micro_batches)

                        if args.train.use_distill:
                            # KL on the same token positions as CE
                        ...
                        ce_loss = unscaled_ce.sum() / (shifted_labels != -100).sum() / len(micro_batches)

                        if args.train.use_distill:
                            # KL on the same shifted token positions as CE
                    ...
                    # 4) KL(student vs teacher) (teacher is target distribution)
                    if args.train.use_distill and args.train.kl_weight > 0:
                        temperature = float(args.train.kl_temperature)
                        if temperature <= 0:
                            raise ValueError(f"kl_temperature must be > 0, got {temperature}")

                        log_q = F.log_softmax(kl_student_logits / temperature, dim=-1)
                        p = F.softmax(kl_teacher_logits / temperature, dim=-1)
                        # token-wise KL: sum over vocab dim, then mask by labels != -100
                        token_kl = F.kl_div(log_q, p, reduction="none").sum(dim=-1)
                        kl_mask = (kl_labels != -100)
                        if torch.any(kl_mask):
                            kl_loss = token_kl.masked_select(kl_mask).mean() / len(micro_batches)
                        else:
                    ...
                    if args.train.use_distill:
                        loss = ce_weight_eff * ce_loss + kl_weight_eff * kl_loss
                    else:
                        loss = ce_loss
            ...
            total_loss, total_ce_loss, total_kl_loss, grad_norm = all_reduce(
                (total_loss, total_ce_loss, total_kl_loss, grad_norm), group=get_parallel_state().fsdp_group
            )
            synchronize()
            if loss_weight_scheduler is not None:
            ...
            # Early stopping (rank 0 decides, then broadcast to all ranks)
                    ...
                    train_metrics.update(
                        {
                            ...
                            "training/kl_loss": total_kl_loss if args.train.use_distill else 0,
                            "training/grad_norm": grad_norm,
                            "training/lr": lr,
                        }
                    )
                    if loss_weight_scheduler is not None:
                        train_metrics["training/ce_weight"] = ce_weight_eff
                        train_metrics["training/kl_weight"] = kl_weight_eff
                        train_metrics.update(loss_weight_scheduler.get_log_state(global_step))
                    if early_stopping_monitor is not None:
                        train_metrics.update(early_stopping_monitor.get_log_state())
