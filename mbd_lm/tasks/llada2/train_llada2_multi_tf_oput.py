...
from mbd_lm.tasks.dataset.data_transform_dmax import process_multi_bd_sft_dmax_oput
...
def main():
    ...
    assert args.train.dual_bd_mode, "train_llada2_multi_bd_oput expects dual_bd_mode: true for Multi-BD."
    assert args.train.same_token_labels, "train_llada2_multi_bd_oput expects same_token_labels: true for DMax OPUT reproduction."
    assert not getattr(args.train, "use_distill", False), "train_llada2_multi_bd_oput is CE-only OPUT; set kl_weight: 0 and use_distill: false."
    ...
    if args.data.data_type == "conversation":
        ...
        explicit_buffer_block_noise_ranges = None
        if args.data.explicit_block1_noise_min >= 0.0:
            explicit_buffer_block_noise_ranges = [
                [args.data.explicit_block1_noise_min, args.data.explicit_block1_noise_max],
                [args.data.explicit_block2_noise_min, args.data.explicit_block2_noise_max],
            ]

        transform = partial(
            process_multi_bd_sft_dmax_oput,
            tokenizer=tokenizer,
    ...
    for epoch in range(start_epoch, args.train.num_train_epochs):
        ...
        for _ in range(start_step, args.train.train_steps):
            ...
            for micro_batch in micro_batches:
                ...
                flags = micro_batch.pop("flag", None)
                if flags is None:
                    raise ValueError("DMax OPUT Multi-BD micro-batch is missing `flag`; rebuild data with duplicated flag=false/true rows.")
                flags = flags.to(get_device_type(), non_blocking=True)
                micro_batch.pop("p_mask", None)

                apply_dmax_on_policy_rollout(
                    model=student_model,
                    micro_batch=student_micro_batch,
                    flags=flags,
                ...
                with model_fwd_context:
                    ...
                    if args.train.same_token_labels:
                        ...
                        ce_loss, ce_metric = compute_global_ce_loss(
                            token_loss=token_ce,
                            labels=gt_labels,
                            p_mask=None,
                    ...
                    loss = ce_weight_eff * ce_loss

                with model_bwd_context:
                    loss.backward()

                total_loss += ce_weight_eff * ce_metric
